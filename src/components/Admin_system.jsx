...
const Admin_system = () => {
  ...
  const sendEtatSuiviData = async (record, actionType, refusalReason = "") => {
    const etatSuiviData = {
      ...
      fin_de_test: actionType === 'approved' ? "Null" : `--`  
    };
  ...
  const renderProjectUrlOptions = () => {
    return (
      <>
        <option value="">{/* Texte pour l'option par défaut */}Choisissez une URL de projet</option>
    ...
    return (
      <>
        ...
        {usersList.map((user, index) => (
          <option key={index} value={user._id + '-' + user.nom}>
            {user.nom}
          </option>
        ))}
      </>
