...
function Moa_test() {
    ...
    const handleFinish = async (isApproved) => {
        ...
        try {
            const response = await axios.post(`${anotherApiUrl}gest_notif`, messageToSend, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
            // Envoyer les données à l'endpoint etat_suivi
            const responseEtatSuivi = await axios.post(`${anotherApiUrl}etat_suivi`, etatSuiviData, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
            ...
            if (!isApproved && refusalReason !== "") {
                await axios.post(`${anotherApiUrl}stocker`, {
    ...
    useEffect(() => {
        axios.get(`${anotherApiUrl}check_MOA`)
            .then((response) => {
