...
function Demande() {
  ...
  useEffect(() => {
    axios
      .get(`${anotherApiUrl}get_project_urls`)
      .then((response) => {
  ...
  useEffect(() => {
    if (projectUrl) {
      axios
        .post(
          `${anotherApiUrl}get_project_info`,
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!valide0 || !valide1 || !valide2 || !valide3) {
      notification.error({
        ...
        description: 'Veuillez cocher toutes les cases de validation',
      });
      return;
    }
    ...
    try {
      ...
      const prodResponse = await axios.post(`${anotherApiUrl}demande_prod`, {
        project_id: projectInfo.project_id,
        project_name: projectInfo.project_name,
        project_url: projectUrl,
      ...
      // Afficher la notification de succès ou d'erreur
      if (prodResponse.data.message) {
        notification.success({
          message: 'Succès',
          description: prodResponse.data.message,
        });
      } else {
        notification.error({
          message: 'Erreur',
      ...
      const notifResponse = await axios.post(`${anotherApiUrl}gest_notif`, messageToSend, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
      ...
      const etatSuiviResponse = await axios.post(`${anotherApiUrl}etat_suivi`, etatSuiviData, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
    ...
    <div>
      ...
      <form onSubmit={handleFormSubmit}>
        ...
        <div className="validation-section">
          <div className="validation-title">Je jure sur l'honneur que ces conditions ont été vérifié et implémenté :</div>
          <div
